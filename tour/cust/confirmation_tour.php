...
$sql = mysqli_query($conn, "SELECT transaksi.*, users.*, wisata.id_wisata, wisata.nama ,wisata.harga FROM transaksi INNER JOIN users ON transaksi.id_user = users.id_user INNER JOIN wisata ON transaksi.id_wisata = wisata.id_wisata where transaksi.id_transaksi='$id_transaksi'");
